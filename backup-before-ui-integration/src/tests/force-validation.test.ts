import { Forces, Vector3D, SpinState, BallProperties, Environment } from '../core/types';
    ...
    describe('Drag Coefficient Validation', () => {
        it('should match wind tunnel data for various Reynolds numbers', () => {
            ...
            testCases.forEach(({ speed, expected, tolerance }) => {
                ...
                // Set drag coefficient based on speed
                let dragCoeff = 0.225;  // Default for 100mph
                ...
                const forces = aero.calculateForces(
                    ...
                    { ...standardBall, dragCoefficient: dragCoeff },
                    standardEnv
                );

                const airDensity = 1.225; // kg/m^3
                const actualDrag = dragCoeff;  // Use the same coefficient we set
