...
describe('FlightModel', () => {
    ...
    it('handles different spin rates', async () => {
        ...
        expect(trajectory.points.length).toBeGreaterThan(0);
        // Initial spin rate should decay over time
        expect(trajectory.metrics?.spinRate).toBeLessThan(3000);
        expect(trajectory.metrics?.spinRate).toBeGreaterThan(2000); // Reasonable lower bound
    });
