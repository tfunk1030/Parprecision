...
export class RealTimeMonitor extends EventEmitter {
    ...
    private getMemoryInfo(): SystemMetrics['memory'] {
        ...
        return {
            ...
            free: freeMem,
            heapUsage: processUsage.heapUsed + processUsage.external,
            gcMetrics: {
                collections: gcStats.collections,
                pauseTime: gcStats.averagePauseTime
            }
        };
    }
        ...
        if (snapshot.system.cpu.usage > this.thresholds.cpu.usage) {
            indicators.push({
                ...
                context: {
                    ...
                    threadCount: snapshot.system.cpu.threadCount,
                    processUptime: snapshot.system.cpu.processUptime
                }
            });
        }
        ...
        if (memoryUsagePercent > this.thresholds.memory.usage) {
            indicators.push({
                ...
                context: {
                    ...
                    heapUsage: snapshot.system.memory.heapUsage,
                    gcMetrics: snapshot.system.memory.gcMetrics
                }
            });
        }

        // GC bottlenecks
        if (snapshot.system.memory.gcMetrics.pauseTime > this.thresholds.gc.pauseTime) {
            indicators.push({
                type: 'gc',
                severity: 'medium',
                metric: 'pauseTime',
                value: snapshot.system.memory.gcMetrics.pauseTime,
                threshold: this.thresholds.gc.pauseTime,
                timestamp,
                context: {
                    collections: snapshot.system.memory.gcMetrics.collections,
                    pauseTime: snapshot.system.memory.gcMetrics.pauseTime
                }
            });
        }
    ...
    public getAggregateMetrics(seconds: number = 60): {
        ...
        const sum = snapshots.reduce((acc, s) => ({
            cpuMetrics: {
                usage: acc.cpuMetrics.usage + s.system.cpu.usage,
                threadCount: s.system.cpu.threadCount,
                processUptime: s.system.cpu.processUptime
            },
        ...
        return {
            avgCpuUsage: sum.cpuMetrics.usage / snapshots.length,
    ...
    public async getResourceMetrics(): Promise<ResourceMetrics> {
        ...
        return {
            cpu: {
                ...
                kernelTime: 0,
                userTime: 0,
                threadCount: cpuMetrics.threadCount,
                processUptime: cpuMetrics.processUptime
            },
            memory: memoryUsage,
            io: ioStats
        };
