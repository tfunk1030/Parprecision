...
export class ThreadManager extends EventEmitter {
    ...
    public async submitTask<T>(task: () => Promise<T>, priority: number = 1): Promise<T> {
        return this.threadPool.submit(task, priority);
    }

    public async getActiveThreadCount(): Promise<number> {
        const stats = this.threadPool.getStats();
        return stats.activeWorkers;
    }

    public async processWorkload<T>(tasks: (() => Promise<T>)[]): Promise<T[]> {
        return Promise.all(tasks.map(task => this.submitTask(task)));
    }

    public async getThreadStats(): Promise<WorkerMetrics[]> {
