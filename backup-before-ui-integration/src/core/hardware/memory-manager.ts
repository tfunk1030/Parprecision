...
export class MemoryManager {
    ...
    public async allocateMemory(size: number, tag?: string): Promise<void> {
        const poolName = this.selectBestPool(size);
        if (!poolName) {
            throw new Error(`No suitable pool found for allocation of size ${size}`);
        }
        ...
        await pool.allocate(size, tag);
        this.monitor.recordAllocation(size);
    }
    ...
    private selectBestPool(size: number): string | null {
        let bestPool: string | null = null;
        let minWaste = Infinity;

        for (const [name, pool] of this.pools.entries()) {
            const waste = pool.getWasteForSize(size);
            if (waste !== null && waste < minWaste) {
                minWaste = waste;
                bestPool = name;
            }
        }
