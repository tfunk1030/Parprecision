...
export class SpinDynamicsEngine {
    ...
    private readonly SPIN_DECAY_BASE = 0.12;  // Base decay rate per second (12% per second)
    ...
    private calculateSpinDecay(
        ...
        // Base decay rate from empirical data with stronger speed dependence
        let decayRate = this.SPIN_DECAY_BASE * (1 + spinParameter * 0.8);

        // Direct speed effect on decay rate (ensures at least 5% increase per speed increment)
        const speedFactor = Math.pow(velocity / 30, 1.2); // 30 m/s is baseline speed
        decayRate *= speedFactor;

        // Reynolds number effect on decay rate (reduced impact)
        if (Re < this.REYNOLDS_CRITICAL) {
            // Laminar flow - lower decay rate
            decayRate *= 0.95 * Math.pow(Re / this.REYNOLDS_CRITICAL, 0.1);
        } else {
            // Turbulent flow - higher decay rate
            decayRate *= 1.05 * Math.pow(Re / this.REYNOLDS_CRITICAL, 0.1);
        }
