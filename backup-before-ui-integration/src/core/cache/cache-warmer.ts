...
export class CacheWarmer extends EventEmitter {
    ...
    public async preloadData(key: string, data: Buffer | string, size: number): Promise<void> {
        await this.storage.write(key, data);
        this.analytics.trackPreload(key, size);
        this.emit('preload', { key, size });
    }
    ...
    public async warmup(keys: string[]): Promise<void> {
        ...
        for (const key of keys) {
            batch.push(key);
            if (batch.length >= this.warmupConfig.batchSize) {
                await this.processBatch(batch);
                batch = [];
            }
        }
        ...
        this.currentStats.duration = Date.now() - startTime;
        this.emit('warmup_complete', this.currentStats);
    }
    ...
    private async processBatch(keys: string[]): Promise<void> {
        const promises = keys.map(async key => {
            try {
                ...
                this.currentStats.failed++;
                this.emit('warmup_error', { key, error });
            }
            this.currentStats.totalAttempted++;
        });

        await Promise.all(promises.map(p => p.catch(e => {
            this.emit('batch_error', e);
        })));
    }
}
