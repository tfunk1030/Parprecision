...
import { CacheRecommendation, MemorySnapshot, ResourceUsage, AnalyticsPeriod } from './types';
...
export class CacheAnalytics extends EventEmitter {
    ...
    private currentPeriod: AnalyticsPeriod = {
        startTime: Date.now(),
        endTime: null,
        patterns: [],
        evictions: [],
    ...
    private periodHistory: AnalyticsPeriod[] = [];
    private memoryHistory: { timestamp: number; used: number; total: number; heapUsage: number }[] = [];
    ...
    public getCurrentPeriod(): AnalyticsPeriod {
        return this.currentPeriod;
    }

    public updateMemoryUsage(snapshot: Omit<MemorySnapshot, 'timestamp'>): void {
        ...
        // Trim old history
        const cutoff = timestamp - 300000; // 5 minutes
        this.memoryHistory = this.memoryHistory.filter(s => s.timestamp >= cutoff);

        this.emit('memory_update', snapshot);
    }

    public recordEviction(key: string, size: number = 0): void {
        this.currentPeriod.evictions.push({
            key,
            size,
            timestamp: Date.now()
        });

        this.emit('eviction', { key, size });
    }

    public rotatePeriod(): AnalyticsPeriod {
        const completedPeriod = { ...this.currentPeriod };
        ...
        // Add to history and trim old periods
        this.periodHistory.push(completedPeriod);
        const cutoff = Date.now() - 86400000; // 1 day
        this.periodHistory = this.periodHistory.filter(p => p.startTime >= cutoff);

        this.emit('period_rotated', completedPeriod);
        return completedPeriod;
    }

    public getHistoricalData(): AnalyticsPeriod[] {
