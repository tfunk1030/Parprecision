...
export class FlightModel {
    ...
    private calculateMetrics(points: TrajectoryPoint[]): ValidationMetrics {
        ...
        const ballSpeed = Math.sqrt(
            firstPoint.velocity.x * firstPoint.velocity.x +
            firstPoint.velocity.y * firstPoint.velocity.y +
            firstPoint.velocity.z * firstPoint.velocity.z
        );
        ...
        return {
            carryDistance,
            totalDistance: carryDistance, // For now, total distance equals carry distance
            maxHeight,
    ...
    private validateMetrics(metrics: ValidationMetrics): boolean {
        ...
        const isTimeValid = metrics.timeOfFlight >= 0 && metrics.timeOfFlight <= 10; // seconds
    ...
    public generateRandomEnvironment(): Environment {
        return {
            ...
            humidity: Math.random(),               // 0-1 (relative humidity)
