...
export function WindCalculator() {
  ...
  useEffect(() => {
    ...
    const calculateWindEffect = () => {
      ...
      setResult({
        distanceEffect: Number(distanceEffect.toFixed(1)),
        lateralEffect: Number(lateralEffect.toFixed(1)),
        totalEffect: Number(Math.sqrt(distanceEffect ** 2 + lateralEffect ** 2).toFixed(1))
      })
    }
  ...
  const handleDirectionChange = (_type: 'wind' | 'shot', degrees: number) => {
    updateConditions({ windDirection: degrees })
  }

  const handleSpeedChange = (speed: number) => {
    updateConditions({ windSpeed: speed })
  }
