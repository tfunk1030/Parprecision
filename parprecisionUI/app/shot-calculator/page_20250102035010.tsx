...
export default function ShotCalculatorPage() {
  ...
  const calculateAdjustments = useCallback(() => {
    ...
    if (!conditions.temperature || !conditions.humidity || 
        !conditions.pressure || !conditions.altitude || 
        !conditions.density || !conditions.windSpeed || 
        !conditions.windDirection) {
      console.error("Missing required weather conditions:", conditions);
      return null;
    }
    ...
    console.log("Weather conditions:", {
      ...
      altitude: `${conditions.altitude}${settings.altitudeUnit}`,
      density: `${conditions.density}kg/m³`,
      windSpeed: `${conditions.windSpeed}mph`,
    ...
    if (altitudeInMeters < 0 || altitudeInMeters > 5000) {
      console.warn(`Invalid altitude value: ${altitudeInMeters}m`)
      return null
    }

    // Air density has primary effect on ball flight
    const densityEffect = (conditions.density - 1.225) * targetYardage * 0.1
    
    // Altitude effect - revised coefficient based on empirical data
    // Thin air reduces drag, but also reduces lift
    const altitudeEffect = -altitudeInMeters * 0.00002 * targetYardage
    ...
    console.debug('Shot adjustments:', {
  ...
  useEffect(() => {
    const refreshInterval = setInterval(() => {
      // Force refresh of environmental conditions
      conditions?.refresh?.();
    }, 5 * 60 * 1000);
